const LIMITATIONS = [
  ...
  '스키마에 없는 속성(예: 신용등급)을 물으면 그 개념이 조용히 빠진 채 다른 컬럼(예: 고객사 목록)이 답으로 나올 수 있습니다. 결과가 질문의 핵심 대상과 맞는지 SQL(source)로 확인하세요. (알려진 한계)',
  '문서마다 값이 다른 설정(예: 제품별 커넥션 풀 수치)을 하나로 물으면, 답변이 한 문서의 값만 단언할 수 있습니다. 실제 근거는 raw_data의 여러 청크에 함께 담기므로 문서별로 확인하세요.',
  '한 질문에 여러 도구가 필요한 다중 의도(예: "계약 금액이랑 담당자 같이")는 규칙 라우터가 한 도구로만 처리하고, 나머지 의도는 별도 질문이 필요합니다.',
]
          ...
          <div>
            ...
            <table className="table">
              ...
              <tbody>
                ...
                <tr><td>라우팅 — LLM 폴백</td><td>약 2.8초 (최대 9.8초)</td></tr>
            ...
            <p style={{ margin: '10px 0 0', fontSize: 12.5, color: 'var(--color-neutral-700)', lineHeight: 1.6 }}>
              표시되는 경과 시간은 브릿지(:8000)를 통해 실제 백엔드가 처리한 실측값입니다. 대부분은 로컬 Ollama(gemma2:9b) LLM 호출 시간이며, 스트리밍이 아니라 완료 후 일괄 응답입니다. 위 값은 중앙값이며, 특히 LLM 폴백은 편차가 커서(표준편차 약 2.5초, 최대 9.8초) 체감 지연이 중앙값의 3배를 넘을 수 있습니다.
            </p>
          </div>
        </div>
